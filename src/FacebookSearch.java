...
public class FacebookSearch {
	 ...
	 * 
	 * Required Permissions: - user_friends - friends_status - friends_photos -
		...
		for (User f : myFriends.getData()) {
			...
			for (List<Checkin> checkins_list : checkins) {
				...
				for (Checkin checkin : checkins_list) {
					...
					treeSet.add(new LocationEntry(checkin.getCreatedTime(),
							checkin.getPlace()));
				}
			}
			...
			for (List<Photo> photos_list : photos) {
				...
				for (Photo photo : photos_list) {
					...
					treeSet.add(new LocationEntry(photo.getCreatedTime(), photo
							.getPlace()));
				}
			}
